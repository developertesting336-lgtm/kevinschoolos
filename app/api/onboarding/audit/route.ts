...
export async function POST(request: NextRequest) {
  try {
    ...
    // RBAC: Only Office Admin and Owner roles can view onboarding tracking
    const role = (dbUser.role || "").toLowerCase().trim();
    if (!["owner", "office_admin"].includes(role)) {
    ...
    auditService.log(
      {
        ...
        result: "SUCCESS",
        details: `Viewed onboarding tracker details for Enrollment ID ${enrollmentId}`,
      },
      request
    );
    ...
    console.error("[Onboarding Audit POST Error]", error);
