...
import { MessageSquare, Phone, Mail, Award, Calendar, User, Info, Plus } from "lucide-react";
...
export function ActivityTimeline({ activities, staffIdToNameMap, onAddActivity }: ActivityTimelineProps) {
    ...
    <div className="bg-card border border-border rounded-xl p-5 shadow-sm space-y-4 select-none">
      <div className="flex items-center justify-between border-b border-border pb-2">
        ...
        {onAddActivity && (
          <Button
            ...
            className="h-7 text-[10px] font-bold gap-1 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground px-2.5 shadow-sm"
          >
      ...
      {activities.length === 0 ? (
        <div className="border border-dashed border-border rounded-xl p-6 text-center">
          <Info className="h-7 w-7 text-muted-foreground/60 mx-auto mb-2" />
          <p className="text-xs font-bold text-muted-foreground">No Activities Recorded</p>
          <p className="text-[10px] text-muted-foreground/85 mt-0.5">There are no documented CRM touchpoints for this lead.</p>
        </div>
      ) : (
          ...
          {activities.map((act) => {
            ...
            return (
              <div key={act.id} className="relative group">
                ...
                <span className="absolute -left-6.75 top-1 h-5 w-5 rounded-full border border-border bg-card flex items-center justify-center shadow-xs">
