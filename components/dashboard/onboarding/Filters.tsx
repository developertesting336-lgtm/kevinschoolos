...
export function Filters({ branches, staff }: FiltersProps) {
  ...
  const currentStatus = searchParams.get("onboardingStatus") || "";
  const currentOwner = searchParams.get("owner") || "";
  const currentEnrollDate = searchParams.get("enrollDate") || "";
  ...
  // Supported onboarding status levels
  const statusOptions = [
    ...
    <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-card border border-border p-4 rounded-xl shadow-xs select-none">
      ...
      <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
        ...
        <div className="relative w-full sm:w-36">
          <input
            type="date"
            value={currentEnrollDate}
            onChange={(e) => handleFilterChange("enrollDate", e.target.value)}
